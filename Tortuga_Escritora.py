...
class Tortuga_Recorre:
    def __init__(self):
        self.alto_total = 400   # Alto fijo de la pantalla
        self.ancho_por_letra = 100  # Ancho que ocupará cada letra
        self.margen = 50  
    ...
    def configuracion_pantalla(self, total_letras):
        ancho_total = (total_letras * self.ancho_por_letra) + (2 * self.margen)
    ...
    def configurar_pos_letra(self, indice,total_letras, ancho_total):
        x = -ancho_total/2 + self.margen + (indice * self.ancho_por_letra) + (self.ancho_por_letra / 2)
    ...
    def Escribir(self, nombre):
        ...
        ancho_total = self.configuracion_pantalla(total_letras)  # Ahora coincide
        for indice, letra in enumerate(nombre.upper()):
            x, y = self.configurar_pos_letra(indice, total_letras, ancho_total)  # Ahora coincide
